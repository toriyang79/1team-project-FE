...
const normalizeBase = (base) => {
  if (!base) return "";
  let val = base.replace(/\/+$/, "");
  val = val.replace(/\/videos?$/i, ""); // .../api/videos -> .../api
  return val;
};
const normalizedEnvBase = normalizeBase(envVideoBase);
const API_VIDEO_BASE_URL = normalizedEnvBase || (isLocalhost ? "/video-api" : "https://shorts-artlion.duckdns.org/api");
