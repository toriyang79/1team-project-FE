...
const HomePage = () => {
  ...
  const TopCard = ({ track, label }: { track: Track; label: string }) => (
    <article className="bg-surface-light rounded-xl p-4 flex gap-3 shadow-sm w-full max-w-[320px]">
      ...
      <section className="flex flex-col gap-4">
        <div className="flex justify-between items-center px-2">
          ...
          <button
            ...
            className="flex items-center gap-2 h-10 px-3 rounded-full bg-surface-light text-sm font-medium hover:bg-black/5 transition-colors"
          >
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(180px,240px))] justify-center gap-4">
          {loading &&
            Array.from({ length: 6 }).map((_, idx) => (
              <div key={idx} className="aspect-[3/4] rounded-xl bg-surface-light animate-pulse" />
            ))}
          {!loading &&
            !error &&
            visibleTracks.map((track) => (
              <article
                key={track.id}
                className="bg-surface-light rounded-xl p-4 flex flex-col gap-2 shadow-sm hover:-translate-y-1 transition-transform w-full max-w-[240px]"
              >
